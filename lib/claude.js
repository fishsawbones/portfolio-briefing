...
3. THESIS CHECK — For any significant mover, assess whether the move is consistent with the original investment thesis or whether the thesis needs revisiting. Be specific. If a rack-level name like Vertiv or Modine dropped 8% on margin compression, say whether that changes the rack-level infrastructure thesis or whether it's noise. If a power name ripped on data center demand commentary, connect it to the portfolio's power bottleneck thesis.
...
5. UPCOMING CATALYSTS — What to pay attention to in the coming week: specific earnings dates, Fed commentary, macro data releases, sector conferences, or catalysts that could move multiple positions. Be specific with dates and names.

6. WATCHLIST — Any positions in the "Watch" basket are unowned names being monitored for entry. For each, report current price, 5-day percent move, 52-week range position, and any material news. Note whether price action is creating a better entry or forcing a decision to chase. Do not include these in Portfolio Snapshot totals or Basket Breakdown — they are not held positions.
